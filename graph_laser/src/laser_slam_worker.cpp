...
void LaserSlamWorker::scanCallback(const sensor_msgs::PointCloud2& cloud_msg_in)
{
  ...
  if (!lock_scan_callback_)
  {
                                      ...
                                      cloud_msg_in.header.stamp, ros::Duration(kTimeout_s)))
    {
        ...
        graph_laser::PointICloud input_cloud;
        pcl::fromROSMsg(cloud_msg_in,input_cloud);
        //是否处理激光点
        ...
        if (process_scan)
        {
            ...
            if (params_.create_filtered_map)
            {
              if (new_fixed_cloud_pcl.size() > 0u)
              {
                ...
                cloud_frame.cloud=new_fixed_cloud_pcl;
                cloud_frame.original_cloud=input_cloud;
                cloud_frame.T_w_linkpose=T_w_sensor;
                cloudData.emplace(new_scan.time_ns,cloud_frame);
              }
            }
        }
    }
...
bool LaserSlamWorker::getLaserTracksServiceCall(
    graph_laser::GetLaserTrackSrv::Request& request,
    graph_laser::GetLaserTrackSrv::Response& response)
{
...
{
    {
        ...
        //遍历每一个有效的分割点云
        for (auto& cloudFrame: cloudData)
        {
          //获取路径中当前时间的位姿
          SE3 new_pose = trajectory.at(cloudFrame.first);

          //获取变换
          SE3 transformation = new_pose * cloudFrame.second.T_w_linkpose.inverse();
          // 转换点云
    //      transformPointCloud(transformation, &cloudFrame.second.cloud);

          //获取对应的点云
          graph_laser::PointCloud* point_cloud=&cloudFrame.second.cloud;
          ...
          // 更新该点云的位姿
          cloudFrame.second.T_w_linkpose = new_pose;
          cloudFrame.second.cloud=*point_cloud;
 ...
 * 获取队列中全部点云
 */
std::vector<graph_laser::PointCloud> LaserSlamWorker::getQueuedPoints()
{
  std::lock_guard<std::recursive_mutex> lock(local_map_mutex_);
  std::vector<graph_laser::PointCloud> new_points;
...
void LaserSlamWorker::getFilteredMap(PointCloud* filtered_map)
{
  ...
  if (params_.separate_distant_map)
  {
    ...
    *filtered_map += distant_map_;
    ...
    std::lock_guard<std::recursive_mutex> lock(full_map_filtered_mutex_);
...
void LaserSlamWorker::getFillMapFiltered(PointCloud* full_filtered_map)
{
    CHECK_NOTNULL(full_filtered_map);
    std::lock_guard<std::recursive_mutex> lock(full_map_filtered_mutex_);
    *full_filtered_map = full_map_filtered_;
}
...
/**
 * @brief LaserSlamWorker::getLocalMapFiltered
 * @param local_map_filtered
 */
void LaserSlamWorker::getLocalMap(PointCloud* local_map)
{
