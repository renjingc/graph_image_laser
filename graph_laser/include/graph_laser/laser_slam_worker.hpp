#ifndef GRAPH_LASER_LASER_SLAM_WORKER_HPP_
#define GRAPH_LASER_LASER_SLAM_WORKER_HPP_
...
#include "graph_laser/GetLaserTrackSrv.h"
...
struct cloudFrame
{
    ...
    graph_laser::PointCloud cloud;
    graph_laser::PointICloud original_cloud;

    //点云位姿
    laser_slam::SE3 T_w_linkpose;
};

typedef std::map<laser_slam::Time, cloudFrame> cloudDatabase;
...
class LaserSlamWorker
{
 ...
 public:
  ...
  //获取局部滤波后的地图
  void getLocalMapFiltered(graph_laser::PointCloud* local_map_filtered);

  //获取局部的地图
  void getLocalMap(graph_laser::PointCloud* local_map);

  //获取滤波后的地图
  void getFilteredMap(graph_laser::PointCloud* filtered_map);

  //获取全局滤波后的地图
  void getFillMapFiltered(graph_laser::PointCloud* full_filtered_map);

  //获取优化后的点云向量
  //获取包含自之后记录的优化点云的向量
  //最后一次调用此方法。 此呼叫清除点云队列。
  std::vector<graph_laser::PointCloud> getQueuedPoints();
 ...
 private:
  ...
  bool getLaserTracksServiceCall(graph_laser::GetLaserTrackSrv::Request& request,
 ...
 private:
  ...
  mutable std::recursive_mutex full_map_filtered_mutex_;
  ...
  graph_laser::PointCloud local_map_;
  std::vector<graph_laser::PointCloud> local_map_queue_;

  //局部滤波后的地图
  graph_laser::PointCloud local_map_filtered_;

  //全局地图
  graph_laser::PointCloud full_map_;
  //全局滤波后的地图
  graph_laser::PointCloud full_map_filtered_;

  //包含远离传感器并假定为固定的地图。
  //如果机器人重新访问相同的环境，则remote_map_and local_map_将是一个
  //彼此之间，每个密度相同。
  //远距离地图
  graph_laser::PointCloud distant_map_;
  ...
  //栅格滤波器
  pcl::VoxelGrid<graph_laser::PclPoint> voxel_filter_;
...
#endif /* GRAPH_LASER_LASER_SLAM_WORKER_HPP_ */
